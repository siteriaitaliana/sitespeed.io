...
module.exports = {
  send(options, group, url, resultPageUrl, time, tsdbType) {
    ...
    const postData = `{"what": "Sitespeed.io", "tags": ${tagsArray}, "data": "${message}", "when": ${timestamp}}`;
    const postOptions = {
    ...
    // If Grafana uses Authorization for the API (ut uses it by default)
    if (options.grafana.apitoken) {
      postOptions.headers.Authorization = 'Bearer ' + options.grafana.apitoken;
    }
