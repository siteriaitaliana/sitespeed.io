...
let merge = require('lodash.merge'),
  ...
  WebPageReplay = require('./webpagereplay'),
...
module.exports = {
  analyzeUrl(url, options) {
    ...
    let engineWpr = new browsertime.Engine({
      xvfb: true,
      docker: true,
      iterations: 1,
      chrome: {
    ...
    const webPageReplay = new WebPageReplay({});
    return webPageReplay
      .startRecord()
      .then(() => engineWpr.start())
      .then(() => engineWpr.run(url, scriptsByCategory))
      .then(() => webPageReplay.stopRecord())
      .then(() => engineWpr.stop())
      .then(() => webPageReplay.startReplay())
      .then(() => engine.start())
      .then(() => engine.run(url, scriptsByCategory))
      .then(() => webPageReplay.stopReplay())
      .then(() => engine.stop());
    /*
     else {
      return engine
